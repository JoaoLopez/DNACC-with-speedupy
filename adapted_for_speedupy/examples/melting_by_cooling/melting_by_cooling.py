...
def figure2a():
    ...
    with open('fig2a.txt', 'w') as f:
        ...
        for beta_DeltaG0strong in range(-40, 7):
            ...
            temp1 = platesXXP.at(2.1 * L)
            temp1.set_reference_now()

            temp2 = [platesXXP.at(h) for h in hArr] 
            VArr = [h.free_energy_density for h in temp2]
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig2a.gp'])
    temp.terminate()
...
def figure2b():
    ...
    with open('fig2b.txt', 'w') as f:
        ...
        for beta_DeltaG0strong in range(-40, 7):
            ...
            set_competing_interactions(platesXX,
                                       beta_DeltaG0strong,
                                       beta_DeltaG0strong + beta_Delta)
            temp11 = platesXX.at(2.1 * L)
            temp11.set_reference_now()

            temp12 = [platesXX.at(h) for h in hArr]
            VArr = [h.free_energy_density for h in temp12]
            hAtMin, minF = min(zip(hArr, VArr), key=operator.itemgetter(1))
            platesXX.at(hAtMin)
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig2b.gp'])
    temp.terminate()
    ...
    with open('fig2c.txt', 'w') as f:
        ...
        for beta_DeltaG0strong in range(-40, 7):
            ...
            set_competing_interactions(platesXXP,
                                       beta_DeltaG0strong,
                                       beta_DeltaG0strong + beta_Delta)
            temp3 = platesXXP.at(2.1 * L)
            temp3.set_reference_now()

            temp4 = [platesXXP.at(h) for h in hArr]
            VArr = [h.free_energy_density for h in temp4]
            for h, V in zip(hArr, VArr):
                f.write('%g\t%g\n' % (h / L, V / (1 / L ** 2)))

            f.write('\n\n')

    temp = subprocess.Popen(['gnuplot', 'plot_fig2c.gp'])
    temp.terminate()
...
def figure3a():
        ...
        for S in (1.5, 1.0, 0.7, 0.5):
            ...
            for beta_DeltaG0strong in range(-30, 3):
                ...
                set_competing_interactions(platesXX,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp5 = platesXX.at(2.1 * L)
                temp5.set_reference_now()

                set_competing_interactions(platesXXP,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp6 = platesXXP.at(2.1 * L)
                temp6.set_reference_now()

                temp7 = [platesXX.at(h) for h in hArr]
                temp8 = [h.free_energy_density for h in temp7]
                minFXX = min(temp8)

                temp9 = [platesXXP.at(h) for h in hArr]
                temp10 = [h.free_energy_density for h in temp9]
                minFXXP = min(temp10)
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig3a.gp'])
    temp.terminate()
...
def figure3b():
        ...
        for Lincrt in (x * L for x in (1.0, 1.2, 1.5, 1.8)):
            ...
            for beta_DeltaG0strong in range(-30, 3):
                ...
                set_competing_interactions(platesXX,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp13 = platesXX.at(2.1 * L)
                temp13.set_reference_now()

                set_competing_interactions(platesXXP,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp14 = platesXXP.at(2.1 * L)
                temp14.set_reference_now()

                temp15 = [platesXX.at(h) for h in hArr]
                temp16 = [h.free_energy_density for h in temp15]
                minFXX = min(temp16)

                temp17 = [platesXXP.at(h) for h in hArr]
                temp18 = [h.free_energy_density for h in temp17]
                minFXXP = min(temp18)
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig3b.gp'])
    temp.terminate()
...
def figure3c():
        ...
        for beta_Delta in (1, 3, 5, 7):
            ...
            for beta_DeltaG0strong in range(-30, 3):
                ...
                set_competing_interactions(platesXX,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp19 = platesXX.at(2.1 * L)
                temp19.set_reference_now()

                set_competing_interactions(platesXXP,
                                           beta_DeltaG0strong,
                                           beta_DeltaG0strong + beta_Delta)
                temp20 = platesXXP.at(2.1 * L)
                temp20.set_reference_now()

                
                temp21 = [platesXX.at(h) for h in hArr]
                temp22 = [h.free_energy_density for h in temp21]
                minFXX = min(temp22)
                
                temp23 = [platesXXP.at(h) for h in hArr]
                temp24 = [h.free_energy_density for h in temp23]
                minFXXP = min(temp24)
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig3c.gp'])
    temp.terminate()
