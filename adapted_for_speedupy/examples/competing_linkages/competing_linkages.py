...
def figure2():
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig2.gp'])
    temp.terminate()
...
def figure4a():
    ...
    with open('fig4a.txt', 'w') as f:
        ...
        for beta_DeltaDeltaG in (-1000, 8, 5, 3):
            ...
            for beta_DeltaG0b in range(-20, 9):
                ...
                temp7 = (plates.at(h) for h in hArr)
                temp7 = (temp8.free_energy_density for temp8 in temp7)
                f.write('%g\t%g\n' %
                        (beta_DeltaG0b,
                         min(temp7) / (1 / L ** 2)))
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig4a.gp'])
    temp.terminate()
...
def figure4b():
    ...
    with open('fig4b.txt', 'w') as f:
        ...
        for beta_DeltaDeltaG in (+1000, -1000, 8, 5, 3):
            ...
            for beta_DeltaG0b in range(-24, 5):
                ...
                temp2 = ((h, plates.at(h)) for h in hArr)
                temp2 = ((temp3, temp4.free_energy_density) for (temp3, temp4) in temp2)
                hAtMin, minF = min(temp2,
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig4b.gp'])
    temp.terminate()
...
def figure5a():
    ...
    temp = subprocess.Popen(['gnuplot', 'plot_fig5a.gp'])
    temp.terminate()
...
def figure5b():
    ...
    with open('fig5b.txt', 'w') as f:
        ...
        for beta_DeltaG0a in (-5.8, -8.7, -11.6, -14.5, -17.4, -20.3):
            ...
            temp5 = [plates.at(h) for h in hArr]
            VArr = [temp6.free_energy_density for temp6 in temp5]
            for (h, V) in zip(hArr, VArr):
                f.write('%g\t%g\n' % (h / L, V / (1 / L ** 2)))

            f.write('\n\n')

    temp = subprocess.Popen(['gnuplot', 'plot_fig5b.gp'])
    temp.terminate()
